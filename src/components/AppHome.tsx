...
export const AppHome = () => {
    ...
    const { networks, favorites, locationStatus, requestLocation, selectNetwork, loading, selectedNetwork } = useCityBikes();
    const favoriteNetworks = networks.filter((network) => favorites.networks.includes(network.id)).slice(0, 3);
    const suggestedNetworks = (favoriteNetworks.length ? favoriteNetworks : networks).slice(0, 3);
